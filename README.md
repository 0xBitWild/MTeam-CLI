...
- 🧠 **AI 友好的数据源** —— 把账户统计、种子搜索/详情、做种/下载、H&R、站内信/公告，以**干净的结构化数据**（JSON/YAML/CSV…）吐出，可直接喂 LLM、接进 Agent，或在终端配合 `jq` 使用。

> 它的前身是单文件脚本 `MT-AutoCheckIn.py`（保留在仓库根作为登录逻辑的参考），现已重构为分层的 `mteam_cli` 包。
...
mteam-cli hnr                          # H&R 记录
mteam-cli messages                     # 站内信
